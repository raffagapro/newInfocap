...
export class SolicitudServicioPage implements OnInit, OnDestroy {
	...
	removeImage(imageIndex: number) {
		this.loadedImages = this.loadedImages.filter(
			(image: any, index: number) => index !== imageIndex
		);
		this.loadedImagesDisplay = this.loadedImagesDisplay.filter(
			(image: any, index: number) => index !== imageIndex
		);
	}
	...
	confirmRequest() {
		...
		formData.append(
			"hours_final",
			moment(this.form.value.eHour).format("HH:mm:ss")
		);
		formData.append(
			"professional_profile_id",
			this.solServ.solicitud.proPerfil_id
		);
		...
		this.lc
			...
			.then((loadingEl) => {
				loadingEl.present();
				this.http
					.post(API + "/client/requestservice", formData, {
