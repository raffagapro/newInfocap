...
let ServicioPagarPage = class ServicioPagarPage {
    ...
    getServiceAditional() {
        let total = this.loadedService.request_cost.reduce((total, cost) => total + (cost.costs_type_id === 2 ? Number(cost.amount_client) : 0), 0);
        return Math.floor(total);
    }
    getTotal() {
        return Math.floor(Number(this.getServiceAditional()) +
